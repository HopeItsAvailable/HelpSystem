...
//	}
	...
	public void registerFirstUser(String username, String password) throws Exception {
		...
		try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(countQuery)) {
			...
			if (userCount == 0) {
				// Encrypt password
				String encryptedPassword = Base64.getEncoder().encodeToString(encryptionHelper
						.encrypt(password.getBytes(), EncryptionUtils.getInitializationVector(username.toCharArray())));
	...
	public void printAllUsers() {
		...
		try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
			...
			System.out.println(
					"ID | Username | Password | Email | First Name | Middle Name | Last Name | Preferred First Name | Is Admin | Is Student | Is Instructor");
			...
			while (rs.next()) {
				...
				// Print user info
				System.out.printf("%d | %s | %s | %s | %s | %s | %s | %s | %b | %b | %b%n", id, username, password,
						email, firstName, middleName, lastName, preferredFirstName, isAdmin, isStudent, isInstructor);
			}
	...
	public void updateUserAccountInfo(String username, String email, String firstName, String preferredFirstName,
	...
	public String displayUsersByAdmin() throws Exception {
		...
		while (rs.next()) {
			...
			userInfo.append("Article ID: ").append(email).append(", Role: ").append(role).append("\n\n");
		}
	...
	public String getAllUsers() {
		...
		try (Statement stmt = connection.createStatement(); ResultSet rs = stmt.executeQuery(query)) {
			...
			// Iterate over the result set and append the necessary user info to the string
			// builder
			while (rs.next()) {
				int id = rs.getInt("id");
				String username = rs.getString("username");
				...
				result.append(String.format("%d | %s | %s | %s%n", id, username, email, roles.toString()));
			}
